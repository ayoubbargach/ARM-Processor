if { ![project_exists {decode}] } {
     project_new {decode}
} else {
     project_open {decode}
}
set_project_settings -cmp {decode}
if { ![project_settings_exist -sim decode] } {
        set_project_settings -sim decode
}
set_global_assignment -section_id {decode} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {decode.vqm}
...
if { [file exists {decode_altera.prop}] } {
  source decode_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {decode} -name {DEVICE} {EP2C70F896I8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {decode_pnr_constraints.sdc}
