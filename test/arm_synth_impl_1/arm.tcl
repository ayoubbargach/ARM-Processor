if { ![project_exists {arm}] } {
     project_new {arm}
} else {
     project_open {arm}
}
set_project_settings -cmp {arm}
if { ![project_settings_exist -sim arm] } {
        set_project_settings -sim arm
}
set_global_assignment -section_id {arm} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {arm.vqm}
...
if { [file exists {arm_altera.prop}] } {
  source arm_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {arm} -name {DEVICE} {EP2C70F896I8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {arm_pnr_constraints.sdc}
