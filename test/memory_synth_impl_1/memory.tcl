if { ![project_exists {memory}] } {
     project_new {memory}
} else {
     project_open {memory}
}
set_project_settings -cmp {memory}
if { ![project_settings_exist -sim memory] } {
        set_project_settings -sim memory
}
set_global_assignment -section_id {memory} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {memory.vqm}
...
if { [file exists {memory_altera.prop}] } {
  source memory_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {memory} -name {DEVICE} {EP2C70F896I8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {memory_pnr_constraints.sdc}
